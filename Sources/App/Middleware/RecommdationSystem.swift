...
//            return req.eventLoop.future(error: Abort(.badRequest,reason:"DB INTERNAL ERROR"))
        ...
        let refData : PythonObject =  ["foo": [0], "bar": [1, 2, 3]]
        let data = Array<Int>(recommand.getRecommandMovies(refData))
//        let test = [String: [Int]] = Dictionary(recommand.getRecommandMovies(refData))!
